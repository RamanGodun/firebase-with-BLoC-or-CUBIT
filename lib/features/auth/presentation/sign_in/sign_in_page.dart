...
import '../../../../core/shared_presentation_layer/utils/error_listener.dart';
...
final class SignInPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      ...
      child: CustomErrorListener<SignInCubit, SignInPageState>(
        failureSelector: (state) => state.failure,
        listenWhen:
            (prev, curr) =>
                prev.status != curr.status && curr.status.isSubmissionFailure,

        onRetry: () => context.read<SignInCubit>().submit(),
        onReset: (context, cubit) {
          cubit
            ..resetStatus()
            ..clearFailure();
        },
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      ...
      builder: (context, child) {
        return CustomErrorListener<SignInCubit, SignInPageState>(
          failureSelector: (state) => state.failure,
          listenWhen:
              (prev, curr) =>
                  prev.status != curr.status && curr.status.isSubmissionFailure,
